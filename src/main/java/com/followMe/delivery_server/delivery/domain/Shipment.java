...
@Entity
@Table(name = "p_shipment", uniqueConstraints = @UniqueConstraint(columnNames = {"delivery_id",
		"sequence"}))
@Getter
...
public class Shipment extends BaseAudit {
	...
	private Shipment(Delivery delivery, int sequence, ShipmentType type, Node fromNode,
			Node toNode) {
	...
	private void checkTypeAndNodes(ShipmentType type, Node fromNode, Node toNode) {
		switch (type) {
			case HUB_TO_HUB -> {
				if (fromNode.getNodeType() != NodeType.HUB ||
						toNode.getNodeType() != NodeType.HUB) {
					throw DeliveryErrorCode.INVALID_SHIPMENT_NODES.toException();
				}
			}
			case HUB_TO_VENDOR -> {
				if (fromNode.getNodeType() != NodeType.HUB ||
						toNode.getNodeType() != NodeType.VENDOR) {
					throw DeliveryErrorCode.INVALID_SHIPMENT_NODES.toException();
				}
			}
	...
	public static Shipment create(Delivery delivery, int sequence, ShipmentType type, Node fromNode,
			Node toNode) {
